...
def extract_statistical_info(file_path):
    ...
    tags = [
        ...
        (r'([Tt]he\s*|\s*\(?)[mM]eans? .*\s*([Ww]as|of|are)\s*(\d+.(?!\D+)[\d),.;]+[s%]?|([\d.]+)[s%]?|\b([+-]?\d+(?:\.\d+)?)\b)\s*', 'Mean value'),
        ...
        (r' \(?\s*[Ss][Dd]\s*\)? ', 'Standard Deviation Value'),
        ...
        # Chi-square test
        (r'[cC]hi[–-]?[Ss]quared? [Tt]ests?', 'test name is Chi-square'),
