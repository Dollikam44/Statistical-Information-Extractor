...
'''
    # the right Code for Extraction of other not tagged Values
    results_dict = OrderedDict()
...
'''#before one Word
    ...
    for sentence in sentences:
        ...
        if tags_found:
            numbers = re.findall(r'\b(\w+)\s+(\d+(?:\.\d+)?)\b', sentence)
            for match in numbers:
                word = match[0]
                number = match[1]
                if number not in matches_found:
                    numbers_not_matched.append((number, word))
        ...
        for sentence in sentences:
            ...
            for pattern, tag in tags:
                ...
                if matches:
                    ...
                    matches_found.extend(matches)
            if tags_found:
                numbers = re.findall(r'\w+\s\d+\.\d+', sentence)  # Updated pattern to include the word before the number
                for number in numbers:
                    word, number = number.split()  # Splitting the matched string into word and number
                    if not any(number in match for match in matches_found):
                        numbers_not_matched.append(f'{word} {number}')  # Appending word and number together
                if numbers_not_matched:
                    numbers_found = numbers_not_matched
                else:
                    numbers_found = []
